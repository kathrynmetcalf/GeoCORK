...
from Functions.Widget_classes import (
    ...
    populate_many_combo_checks, populate_model_checks, delete_data, scroll_to_record, get_columns, get_table_from_view,
    find_current_sub_items, get_record_row, get_id_from_name, add_tree_popup, save_expanded_state, get_readable_header,
    get_name_from_id, find_tree_model, get_view_from_table, TreeSortFilterProxyModel, populate_tree_model_checks,
    columns_as_list, show_loading_dialog, close_loading_dialog, CheckableTreeView, CheckableSQLiteTableModel,
    get_edit_view_from_table, common_strings
)
...
class EditView(QtW.QDialog):
    """
    Class to edit data in a table generated by a view query (Samples, Grains, Spots, UPbAnalyses, Columns, References)
    """
    def __init__(self, parent_window, table_name, **kwargs):
        ...
        self.where_table_item_ids: list = []
        self.table_item_ids: list = []
        for key, value in kwargs.items():
            setattr(self, key, value)
        if isinstance(self.parent_id, str):
            self.parent_id = int(self.parent_id)
        if not self.where_table_item_ids and self.table_item_ids != []:
            self.where_table_item_ids = self.table_item_ids
        elif not self.table_item_ids and self.where_table_item_ids != []:
            self.table_item_ids = self.where_table_item_ids
        ...
        self.edit_view = self.edit_tableView
    ...
    def connect_table_signals(self):
        ...
        self.edit_tableView.frozen_table_view.installEventFilter(self)
        self.edit_tableView.selectionModel().currentChanged.connect(self.on_index_change)
        self.edit_tableView.frozen_table_view.selectionModel().currentChanged.connect(self.on_index_change)
        ...
        self.edit_tableView.frozen_table_view.doubleClicked.connect(self.display_widget)
    ...
    def disconnect_table_signals(self):
        ...
        try:
            ...
            self.edit_tableView.frozen_table_view.removeEventFilter(self)
        except ValueError:
            pass
        try:
            self.edit_tableView.selectionModel().currentChanged.disconnect(self.on_index_change)
            self.edit_tableView.frozen_table_view.selectionModel().currentChanged.disconnect(self.on_index_change)
        except (TypeError, AttributeError) as e:
            pass
        try:
            self.edit_tableView.customContextMenuRequested.disconnect(self.show_context_menu)
            self.edit_tableView.frozen_table_view.customContextMenuRequested.disconnect(self.show_context_menu)
        except TypeError:
            pass
        try:
            self.edit_tableView.selectionModel().currentRowChanged.disconnect(self.on_row_change)
            self.edit_tableView.frozen_table_view.selectionModel().currentRowChanged.disconnect(self.on_row_change)
        except (TypeError, AttributeError) as e:
            pass
        try:
            self.edit_tableView.doubleClicked.disconnect(self.display_widget)
            self.edit_tableView.frozen_table_view.doubleClicked.disconnect(self.display_widget)
        except TypeError:
            pass
    ...
    def create_model(self):
        ...
        if len(self.where_table_item_ids) > 0:
            self.table_item_ids = self.where_table_item_ids
            if len(self.where_table_item_ids) == 1:
                sql_where_str = f'= {self.where_table_item_ids[0]}'
            else:
                sql_where_str = f'IN {tuple(self.where_table_item_ids)}'
    ...
    def eventFilter(self, object, event):
        ...
        if objects_statement:
            ...
            if object is self.lineEdit:
                if event.type() == QtC.QEvent.Type.FocusOut and self._isApplicationFocused:
        ...
        # elif isinstance(object, QtW.QTableView):
        #     self.edit_view = object
        return super().eventFilter(object, event)

    def show_context_menu(self, pos):
        # Check if right-clicked on an existing combo box in the cell
        if isinstance(self.sender(), QtW.QTableView):
            self.edit_view = self.sender()
        else:
            self.edit_view = self.edit_tableView
        indexes = self.edit_view.selectedIndexes()
        if not indexes:
            return
        ...
        action = menu.exec(self.edit_view.viewport().mapToGlobal(pos))
        if action is None:
            return
            ...
            if self.lineEdit is not None:
                if len(selected_rows) > 1 and indexes[0].column() == self.name_column:
                    ...
                    view_model = self.edit_view.model()
            ...
            for index in indexes:
                row = index.row()
                column = 0
                id = self.edit_view.model().index(row, column).data(QtC.Qt.ItemDataRole.DisplayRole)
            ...
            if merge_dlg.exec() == QtW.QDialog.DialogCode.Accepted:
                ...
                self.where_table_item_ids = [item_id for item_id in self.where_table_item_ids if item_id not in ids_to_delete]
    ...
    def display_widget(self):
        if isinstance(self.sender(), QtW.QTableView):
            self.edit_view = self.sender()
        if len(self.edit_view.selectedIndexes()) == 0:
            return
        elif len(self.edit_view.selectedIndexes()) > 1:
            logger_setup.get_logger().error('Right-click with selected values in single column to edit multiple selections')
            return
        proxy_index = self.edit_view.selectedIndexes()[0]
    ...
    def determine_widget(self, model_index):
        ...
        item_ids = []
        for proxy_index in self.edit_view.selectedIndexes():
            row = proxy_index.row()
    ...
    def update_model_data(self, show_columns: list, item_ids: list) -> bool:
        ...
        model_indexes = [self.proxy_model.mapToSource(proxy_index) for proxy_index in self.edit_view.selectedIndexes()]
    ...
    def create_lineedit(self):
        ...
        self.lineEdit = QtW.QTextEdit()
        if len(self.edit_view.selectedIndexes()) > 1:
            self.edit_index = QtC.QModelIndex()
            text_items = []
            for index in self.edit_view.selectedIndexes():
            ...
            self.edit_index = self.edit_view.selectedIndexes()[0]
    ...
    def display_lineedit(self):
        ...
        close_loading_dialog('Loading', f'Loading...')
        self.lineEdit.installEventFilter(self)
        # self.lineEdit.focusOut.connect(self.save_lineedit_data)
        # self.lineEdit.editingFinished.connect(self.save_lineedit_data)
        self.edit_view.setIndexWidget(self.edit_view.selectedIndexes()[0], self.lineEdit)
        self.lineEdit.setFocus()

    def save_lineedit_data(self):
        if not self._isApplicationFocused:
            return
        logger_setup.get_logger().info('Saving data from line edit')
        if self.lineEdit is not None:
            # edit_value = self.lineEdit.text()
            edit_value = self.lineEdit.toPlainText()
            if (len(self.edit_view.selectedIndexes()) == 1 and self.edit_index.isValid() and
                    self.edit_index.data(QtC.Qt.ItemDataRole.DisplayRole) == edit_value):
                logger_setup.get_logger().info(f'No changes to data')
                self.destroy_lineedit()
                return
            ...
            for model_index in model_indexes:
                ...
                if self.model.setData(model_index, edit_data, QtC.Qt.ItemDataRole.EditRole):
                    if self.edit_view.currentIndex() == self.edit_index:
                        self.tabbed_from_editor = False
                else:
    ...
    def destroy_lineedit(self):
        try:
            ...
            pass
        self.edit_view.setIndexWidget(self.edit_index, None)
        self.lineEdit = None
        self.edit_index = QtC.QModelIndex()
        self.common_edit_string = None
        self.edit_view.setFocus()
    ...
    def create_dropdown(self):
        ...
        if len(self.edit_view.selectedIndexes()) > 1:
            ...
            self.combo_index = self.edit_view.selectedIndexes()[0]
    ...
    def display_dropdown(self):
        self.edit_view.setIndexWidget(self.edit_view.selectedIndexes()[0], self.combo)
    ...
    def save_dropdown_data(self):
        ...
        if updated:
            ...
            for model_index in model_indexes:
                if model_index in self.model.edited_indexes:
                    self.model.edited_indexes.remove(model_index)
        if self.edit_view.currentIndex() == self.combo_index:
            self.tabbed_from_editor = False
        self.combo = combo
    ...
    def destroy_dropdown(self):
        ...
        try:
            ...
            pass
        self.edit_view.setIndexWidget(self.combo_index, None)
        if self.combo is not None:
    ...
    def determine_edit_table(self, selected_ids):
        if self.combo and self.combo_index.isValid():
            ...
            selected_index = self.edit_view.selectedIndexes()[0]
    ...
    def clear_data(self):
        ...
        for column, ids in columns.items():
            ...
            if not edit_indexes:
                ...
                rollback_savepoint('before_clear')
                return
            if edit_indexes != self.edit_view.selectedIndexes():
                ...
                try:
                    ...
                    pass
                # Clear the selection
                self.edit_view.selectionModel.clearSelection()
                selection = QtC.QItemSelection()
                # Select the indexes to edit
                for index in edit_indexes:
                    selection.select(index, index)
                self.edit_view.selectionModel.select(selection, QtC.QItemSelectionModel.SelectionFlag.Select)
                # Reconnect the selection model signal
                self.edit_view.selectionModel().selectionChanged.connect(self.on_index_change)
                self.edit_view.selectionModel().currentRowChanged.connect(self.on_row_change)
