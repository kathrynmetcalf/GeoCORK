...
def Puetz_importer():
    ...
    """
    full_data = '/Users/kametcalf/Zotero/storage/523QXB2R/DB3_2023.xlsx'
    db = '/Users/kametcalf/Documents/Research/GeoChron_non_git/Puetz_et_al_2024-3.db'
    reference_dict = {}
    ref_sample_dict = {}
    ...
    for i in range(1, rows):
        ...
        # Spot
        spot_name = f'{upb_df.iloc[i, 1]}_calculated'
        ...
        # spot_context
        spot_contexts = [upb_df.iloc[i, 2], f'Concordance class {upb_df.iloc[i, 31]}']
        for spot_context in spot_contexts:
        ...
        # ages
        pb6_u8_age = upb_df.iloc[i, 20]  # calculated
        if pd.isna(pb6_u8_age):
            pb6_u8_age = None
        pb6_u8_age_err = upb_df.iloc[i, 21]
        if pd.isna(pb6_u8_age_err):
            pb6_u8_age_err = None
        pb7_u5_age = upb_df.iloc[i, 22]  # calculated based on 238U/235U = 137.818
        if pd.isna(pb7_u5_age):
            pb7_u5_age = None
        pb7_u5_age_err = upb_df.iloc[i, 23]  # calculated based on 238U/235U = 137.818
        if pd.isna(pb7_u5_age_err):
            pb7_u5_age_err = None
        pb7_pb6_age = upb_df.iloc[i, 24]
        if pd.isna(pb7_pb6_age):
            pb7_pb6_age = None
        pb7_pb6_age_err = upb_df.iloc[i, 25]
        ...
        try:
            ...
            cursor.execute(f'''SELECT UPbAnalysisID FROM UPbAnalyses WHERE 
                            ...
                            iif(:spot_size_unit_id IS NULL, "SpotSizeUnitID" IS NULL, "SpotSizeUnitID" = :spot_size_unit_id) AND
                            iif(:rejected IS NULL, "Rejected" IS NULL, "Rejected" = :rejected)''',
                           {'spot_id': spot_id, 'reference_id': reference_id, 'lab_facility_id': lab_facility_id,
                            ...
                            'spot_size_unit_id': spot_size_unit_id, 'rejected': rejected})
            ...
            if analysis_id is None:
                ...
                cursor.execute(f'''INSERT INTO UPbAnalyses (SpotID, "ReferenceID", LabFacilityID, InstrumentID, 
                            ...
                            "Concordance", "ConcordanceFormatID", SpotSize, SpotSizeUnitID, Rejected) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                         (spot_id, reference_id, lab_facility_id, instrument_id, method_id, pb7_pb6,
                            ...
                            concordance_format_id, spot_size, spot_size_unit_id, rejected))
                conn.commit()
                cursor.execute(f'''SELECT UPbAnalysisID FROM UPbAnalyses WHERE 
                            iif(:spot_id IS NULL, SpotID IS NULL, SpotID = :spot_id) AND 
                            iif(:reference_id IS NULL, "ReferenceID" IS NULL, "ReferenceID" = :reference_id) AND
