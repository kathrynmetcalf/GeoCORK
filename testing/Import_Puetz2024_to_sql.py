...
def Puetz_importer():
    ...
    # reference_dict_df = pd.DataFrame(columns=['ReferenceNumber', 'ReferenceID'])
    # reference_dict_df['ReferenceNumber'] = reference_df['Ref No.']
    # reference_dict_df['ReferenceID'] = reference_sql_df['ReferenceID']
    #
...
# def add_children(parent_id_dictionary, parent_row_dictionary, parent_id, children, sql_df):
#     for child in children:
#         if pd.isnull(child):
#             continue
#         # For a tree structure in GeoCORK, name is always column 3 and ID is always column 0
#         child_parent_id = sql_df.loc[sql_df[3] == child, 0].values[0]
#         if child_parent_id != parent_id:
#             if child not in parent_id_dictionary:
#                 child_parent_row = children.index(child)
#                 parent_id_dictionary[child] = int(child_parent_id)
#                 parent_row_dictionary[child] = child_parent_row
#             child_children =
    ...
    lat_df = pd.DataFrame(columns=table_properties['GPSLocations'])
