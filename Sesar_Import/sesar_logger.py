...
def stop_sesar_logger() -> None:
    """
    Flush and tear down the current SESAR logger session.

    Called at the end of every SESAR import session (i.e. when the user
    exits the ImportFromSesar window). Safe to call multiple times — all
    state is reset to None, so a subsequent setup_sesar_logger() call will
    build a fresh logger writing to a new timestamped log file.

    What this cleans up:
      - Writes a session-end marker to the current log file
      - Stops and discards the background queue listener
      - Removes all handlers from the logger instance
      - Clears module-level singletons so the next session starts fresh
    """
    global _queue_listener, _logger, _log_file_path

    # No-op if nothing is set up (e.g. called twice, or called before any
    # logging ever happened).
    if _logger is None and _queue_listener is None:
        return

    # Write a closing marker, matching the session-start banner in
    # setup_sesar_logger(). Wrapped in try/except because we're on the
    # teardown path and mustn't raise.
    if _logger is not None:
        try:
    ...
    # Stop the listener so the file handler flushes any pending records to
    # disk. QueueListener.stop() blocks until its worker thread exits, so
    # after this line the log file is fully written.
    ...
    # Remove every handler from the logger and close each one. Without this,
    # a later setup_sesar_logger() would stack fresh handlers on top of the
    # old ones; because a logger is looked up by name (getLogger("Sesar…")),
    # Python returns the SAME instance every time — so lingering handlers
    # from the previous session would still be attached.
    if _logger is not None:
    ...
    # Clear the singletons so the guard at the top of setup_sesar_logger()
    # ("if _logger is not None: return _logger") doesn't short-circuit the
    # next session and leave us with a stale, handler-less logger.
    _logger = None
    _log_file_path = None
